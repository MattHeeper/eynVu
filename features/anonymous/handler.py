...
from utils.state import (
    ...
    STATE_WAITING_MESSAGE, STATE_WAITING_CONFIRMATION
)
...
async def handle_anonymous_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle incoming message from user (for anonymous sending)"""
    user_id = update.effective_user.id
    ...
    keyboard = [
        [
            InlineKeyboardButton("✅ آره", callback_data="confirm_send_yes"),
            InlineKeyboardButton("❌ نه", callback_data="confirm_send_no")
        ],
        [InlineKeyboardButton("✏️ ویرایش پیام", callback_data="edit_message")]
    ]
...
async def confirm_send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        anon_message = AnonymousMessage(
            ...
            sender_message_id=query.message.message_id
        )
...
async def edit_message_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Go back to waiting message state
    set_state(user_id, STATE_WAITING_MESSAGE, {
        "recipient": state["data"]["recipient"]
    })
    
    await query.edit_message_text(
        "✏️ پیام جدید خود را بفرستید:",
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("❌ لغو", callback_data="cancel")
        ]])
    )
